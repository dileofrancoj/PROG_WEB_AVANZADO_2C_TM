...
async function insertProducto(obj) {
    try {
        ...
        // console.log("Se inserto el producto con id:" + rows.insertId);
...
async function deleteProducto(id_producto, id_cliente) {
    try {
        let query = "delete from ?? where id_producto = ?";
        const rows = await pool.query(query,[process.env.TABLA_PRODUCTOS,id_producto, id_cliente]);
        return rows; 
    } catch (error) {
        throw error; 
